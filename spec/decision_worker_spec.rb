...
describe Ntswf::DecisionWorker do
  ...
  let(:event) do
    double("Event", attributes: attributes, event_type: event_type,
  ...
  describe "processing a decision task" do
    ...
    describe "handling event" do
      ...
      context "with given interval" do
        ...
        describe "TimerFired" do
          ...
          it "should pass on mandatory attributes" do
            task.should_receive(:continue_as_new_workflow_execution).with(
                hash_including(attributes_hash))
            worker.process_decision_task
          end
        end
      end
